...
def compare_excel(compare, to):
    ...
    global path_all
    # 获取第一个文件名称
    new_path = re.findall(r'[^\\/:*?"<>|\r\n]+$', name)
    path_all = path + '_compare_' + new_path[0]
    ...
    try:
        writer = pd.ExcelWriter(path_all, engine='xlsxwriter')
        ...
        tkinter.messagebox.showwarning(title='Hi', message='请关闭xlsx文件后在进行操作')
...
def on_click():
    ...
    try:
        ...
        tkinter.messagebox.showinfo(title='success', message="Compare done, please see the result: " + path_all)
        res()
        b3.configure(text="Start Compare", state=NORMAL)
...
def res():
    global path_all
    l_res = Label(root, text=path_all)
    l_res.pack()
